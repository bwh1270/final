...
class ScoreMap:
    ...
    '''
    def __init__(self, target_x, target_y, publisher_name):
        # Node name
        ...
        # Subscriber
        rospy.Subscriber('/carrot_team/uav_xy', Point, self.get_pose)
        rospy.Subscriber('/carrot_team/pcl2xy', xyz, self.get_wall)
    ...
    def cal_score_map(self):
        while not rospy.is_shutdown:
            ...
            while len(self.wall_x) > 0:
                ...
                for i in range(-1 * self.safety_block, self.safety_block + 1):
                    for j in range(-1 * self.safety_block, self.safety_block + 1):
                        ...
                        tmp_y + j <= self.y_lower_limit or tmp_y + j >= self.y_upper_limit:
                            continue
            ...
            while len(self.arr_q) > 0:
                ...
                if tmp_x == self.drone_box_x and tmp_y == self.drone_box_y:
                    if self.is_print:
                        print("Meet drone")
                    break
...
if __name__ == "__main__":
    target0 = ScoreMap(-2.2+4.2, -1.06+3, 'target0') # Target in m
    """
if False:
    ...
    target0 = ScoreMap(poi_list[0][0], poi_list[0][1], 'target0') # Target in m
    target1 = ScoreMap(poi_list[1][0], poi_list[1][1], 'target1') # Target in m
    target2 = ScoreMap(poi_list[2][0], poi_list[2][1], 'target2') # Target in m
    target3 = ScoreMap(poi_list[3][0], poi_list[3][1], 'target3') # Target in m
    target4 = ScoreMap(poi_list[4][0], poi_list[4][1], 'target4') # Target in m
    target5 = ScoreMap(poi_list[5][0], poi_list[5][1], 'target5') # Target in m
    

    target0 = ScoreMap(poi_list[0], 'target0') # Target in m"""
