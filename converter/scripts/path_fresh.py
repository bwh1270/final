...
class Path:
    def __init__(self, poi_len: int):
        ...
        self.absolute_location = np.array([-2.4, 0.5,1.2]) # in meters
        self.current_idx = np.array([0, 0])
        self.absolute_orientation = np.array([0, 0, 0, 1])
        self.set_point = 0
        ...
        self.home = np.array([-2.4, 0.5, 1.2])  # home poi (single poi in meters)
    ...
    def set_home(self):
        ...
        if (X_ > self.idx_map_x or X_ < 0) or (Y_ > self.idx_map_y or Y_ < 0) or (x_ < 0 or x_ > self.map_x) or (y_ < 0 or y_ > self.map_y):
    ...
    def is_arrived_at_poi(self):
        print('shape of poi_to_go :', np.shape(self.poi_to_go))
        ...
        print("curent_idx", np.shape(self.current_idx))
        self.current_idx = np.reshape(self.current_idx, (1,2))
        print("curent_idx", np.shape(self.current_idx))
        curr_x = int(self.current_idx[0][0])
        curr_y = int(self.current_idx[0][1])
        ...
        delta_meter_x = abs(updating_x - poi_meter_x)
        delta_meter_y = abs(updating_y - poi_meter_y)
        #print(f"(poi_x, poi_y): ({poi_x}, {poi_y})")
        #print(f"(curr_x, curr_y): ({curr_x}, {curr_y})")
        #print(f"(poi_meter_x, updating_y): ({poi_meter_x}, {poi_meter_y})")
        #print(f"(updating_x, updating_y): ({updating_x}, {updating_y})")
    ...
    def set_target_poi(self):
        ...
        if len(idx) > 0:
            ...
            self.target_poi_idx = np.array([np.round((self.target_poi[0] + self.map_x / 2) * self.scale), np.round((self.target_poi[1] + self.map_y / 2) * self.scale)])
    ...
    def set_set_point(self, flag):
        ...
        if self.slam_map[self.set_point_x, self.set_point_y] == 1:
            self.need_slam_flag = False
            self.set_point_list.append([self.set_point_x, self.set_point_y, self.home[2]]) # for making trajectory
    ...
    def check_poi_arrived(self):
        ...
        if ((delta_x == 0) and (delta_y == 0)):
            print("Finish moving to target_poi_idx")
            return True
        ...
        elif self.need_slam_flag == True:
            print("need slam map update")
            return True
    ...
    def lower_controller(self, point):
        ...
        x_meter, y_meter, _ = self.idx2m(point)
        ...
        while True:
            ...
            delta_x = abs(updating_x - x_meter)
            delta_y = abs(updating_y - y_meter)
            if (delta_x < 0.2) and (delta_y < 0.2):
                print("Arrived at desired one step; Distance; good")
                break
            if time.time() - time_check > 10:
                print("Error! Arrived at desired one step; Time out")
                break
            #sleep(0.5)
