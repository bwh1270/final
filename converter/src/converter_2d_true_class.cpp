...
Converter2DTrue::Converter2DTrue(ros::NodeHandle *nh)
{
	...
	// Subscriber
	vicion_pose_sub_ = nh->subscribe("/falconblack/vrpn_client/estimated_odometry", 100, &Converter2DTrue::vicon_pose_callback, this);
	octomap_pcl_sub_ = nh->subscribe("/octomap_point_cloud_centers", 100, &Converter2DTrue::octomap_pcl_callback, this);
...
double Converter2DTrue::fitting_to_grid(double x)
{
	double rounded;
	double int_part;
...
void Converter2DTrue::vicon_pose_callback(const nav_msgs::Odometry::ConstPtr &msg)
{
	double vicon_xy[2];
	...
	for (int i=0; i<2; ++i) {
		uav_grid_position_[i] = Converter2DTrue::fitting_to_grid(vicon_xy[i]);
	}
...
void Converter2DTrue::octomap_pcl_callback(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
	...
	for (; iter_x != iter_x.end(); ++iter_x, ++iter_y, ++iter_z)
	{
        ...
        if ((obstacle_xyz[2] < 1) || (obstacle_xyz[2] > 2)) {
            ...
            point_msg.x = (Converter2DTrue::fitting_to_grid(obstacle_xyz[0]) + vicon_bias_x_) * 2;
            point_msg.y = (Converter2DTrue::fitting_to_grid(obstacle_xyz[1]) + vicon_bias_x_) * 2;
            point_msg.z = -1;
	...
	Converter2DTrue::octomap_to_grid();
...
double Converter2DTrue::deg2rad(double deg) {
	double rad = (deg/180) * CARROT_PI;
	return rad;
}


void Converter2DTrue::octomap_to_grid()
{
	int n;
	int N;
	double L;
	...
	for (int i = 0; i<N; ++i) {
		for (int j=0; j<2; ++j) {
			points_array_grid[i][j] = Converter2DTrue::fitting_to_grid(points_array[i][j]);
		}
	}
	...
	for (int i=0; i<N; ++i) {
		for (int k=1; k<=line_grid_res_; ++k) {
			...
			fitted_line_grid[k+line_grid_res_*i][0] = Converter2DTrue::fitting_to_grid(line_grid_one[0]);
			fitted_line_grid[k+line_grid_res_*i][1] = Converter2DTrue::fitting_to_grid(line_grid_one[1]);	
		}
	}
	...
	for (int i=0; i<voxel_grid_map_size_; ++i) {
		...
		fov_voxel_grid_map_32_msg.data[i] = 1;  /* here! */
	}
